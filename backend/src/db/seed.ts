...
async function backfillPublicUuids(table: string) {
  const col = getCollection(table)
  const missing = await col.find({
    $or: [{ uuid: { $exists: false } }, { uuid: null }, { uuid: '' }],
  }).toArray()
  for (const doc of missing) {
    await col.updateOne({ _id: doc._id }, { $set: { uuid: newPublicUuid() } })
  }
  const existing = await col.find({ uuid: { $type: 'string' } }).toArray()
  for (const doc of existing) {
    if (typeof doc.uuid === 'string' && !isPublicUuid(doc.uuid)) {
      await col.updateOne({ _id: doc._id }, { $set: { uuid: newPublicUuid() } })
    }
  }
}
...
export async function seedMongo() {
  const db = getMongoDb()
  await backfillPublicUuids(schema.dramas.__name)
  await backfillPublicUuids(schema.episodes.__name)
  await getCollection(schema.dramas.__name).createIndex({ uuid: 1 }, { unique: true })
  await getCollection(schema.episodes.__name).createIndex({ uuid: 1 }, { unique: true })
  ...
  for (const seed of stylePresetSeeds) {
    ...
    if (exists) {
      if (seed.value === '3d') {
        await presets.updateOne({ _id: exists._id }, { $set: { name: seed.name, prompt: seed.prompt, description: seed.description, updatedAt: ts } })
      }
      continue
    }
