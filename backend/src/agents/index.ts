...
与视频提示词同一请求时遵守 Skill storyboard-image：只画第一个【镜头N】的单帧；有 image_refs 时必须写「第一张图 / 第二张图」锁定 Gemini 前面附上的人物／场景／道具图像素，不要写 @角色名 或 <IMAGE_REF_N>，不要时间轴、不要旁白配音、不要复制 video_prompt。
