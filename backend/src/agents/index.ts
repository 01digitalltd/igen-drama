...
- 视频提示词必须遵守项目视觉风格（用户消息【视觉风格】）：3D Chibi 保持 3D Q版 CG（头大身小），不要写成真人实拍或半写实成人比例。@参考图只锁外形／包装／Logo，必须写明转成该画风，不要按实拍照片还原。品牌Logo 按用户消息【Logo放置】：全片每一镜或只在最后一镜。图片最终提示词与 image_prompt 也必须写进该画风：3D Chibi 禁止「电影质感」「真人皮肤」；保存工具仍会再注入风格锁
