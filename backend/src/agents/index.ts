...
- 视频提示词必须遵守项目视觉风格（用户消息【视觉风格】）：3D Chibi 保持 3D Q版 CG（头大身小），不要写成真人实拍或半写实成人比例。@参考图只锁外形／包装／Logo，必须写明转成该画风，不要按实拍照片还原。品牌Logo 按用户消息【Logo放置】：全片每一镜或只在最后一镜。图片最终提示词的风格词仍由保存工具自动注入，不要自行发明另一种画风
