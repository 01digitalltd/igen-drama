...
export const REALISTIC_FACE_GRID_IMAGE_PROMPT =
  'overlay a 6x6 white grid across the entire image (pure white #FFFFFF, 100% opacity, 12px-thick lines); six columns by six rows spanning the full frame from edge to edge, covering people, clothing, background and all other content; the white grid must stay fully opaque and clearly visible'

export const REALISTIC_FACE_GRID_VIDEO_PROMPT =
  '去掉参考图整张画面上的白色6×6网格（不透明度100%、线宽约12px），还原无网格的自然画面，不得残留格子或白色线。Remove the white 6x6 grid covering the entire reference image (100% opacity, 12px-thick lines); restore the natural scene with no lattice overlay.'

const LEGACY_IMAGE_FACE_GRID = /(?:,\s*)?on every visible human face only, overlay (?:an ultra-fine orange mesh grid|a 6x6 white grid)[^.]*(?:\.|$)/gi
const HAS_IMAGE_FACE_GRID = /整张(图|画面).*白色\s*6\s*[×x]\s*6|6\s*[x×]\s*6 white grid across the entire image/i
const HAS_VIDEO_FACE_GRID_REMOVAL =
  /去掉参考图整张画面上的白色6\s*[×x]\s*6网格|Remove the white 6x6 grid covering the entire reference image/
