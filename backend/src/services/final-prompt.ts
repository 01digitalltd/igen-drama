    ...
    const drafted = await withTimeout(
      generateStructuredPrompt(
        ...
        [
          ...
          '若本剧是写实真人风格：整张图必须覆盖白色 9×9 网格（纯白、不透明度 100%、线宽 12px，铺满全画幅）；网格线必须切开人物五官（眉、眼、鼻、口、耳），把五官分拆到不同格子，不得让完整五官落在同一格；网格必须完全不透明、清晰可见。',
  ...
  try {
    ...
    const drafted = await withTimeout(
      generateStructuredPrompt(
        ...
        scene.dramaId,
        [
          `为场景「${scene.location}」(scene_id=${scene.id}) 写固定机位广角镜头的最终提示词。`,
          '必须包含前景、中景、后景分层，以及出入口；纯中文单段，不要风格词、不要英文。',
          `地点：${scene.location}；时间：${scene.time || ''}；描述：${scene.prompt || ''}；光影：${scene.lighting || ''}`,
          'Return JSON {"prompt":"..."} only.',
        ].join('\n'),
        opts,
      ),
