...
当同时注入广告技能时，以上「保留核心情节」不适用。原文是 brief 与产品事实来源，不是必须保留的剧情。

- 产品名、品牌、价格、功效、用户没给的信息：仍不得编造
- 情节机制可彻底重构：内部从创意情节库选 1 个主机制（至多 1 个辅助），按五拍写成广告分场剧本
- 仍必须遵守格式化剧本格式；禁止输出构想列表、markdown 分镜表或镜头语言
- 短剧项目忽略本段
