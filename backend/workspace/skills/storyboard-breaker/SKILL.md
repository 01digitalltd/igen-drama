...
- 每个段落时长必须落在 `read_storyboard_context.video_generation.duration_min–duration_max` 秒（建议 `typical_shot` 秒），内部承载 **2-4 个子镜头**
- 禁止写出超过 `duration_max` 的 `duration`（例如 Gemini Omni 上限 10 秒时不得写 12–15 秒）
...
2. **节拍识别**：先识别剧本的叙事节拍——剧本中的【开场】【触发】【高潮】【收尾】等标记，或叙事转折点（地点转移、规则揭示、情绪爆发、反转）。节拍可在同一段落内用【镜头N】子镜头表达；只有在不超出总时长与段数上限时才切段。同一因果链（铺垫-发生-反应）优先归入同一段落
3. **总量锚定（硬上限）**：若 `video_generation.target_duration_seconds` 有值，全部分镜 `duration` 之和不得超过 `max_total_seconds`，段落数必须落在 `estimated_shot_count.min–max`（建议 `typical`），每段优先 `suggested_shot_duration`。宁可把多个节拍压进同一段落，也不要多拆。没有目标秒数时，才用剧本字数 ÷ 500字/分钟估算
4. **段落内拆子镜头**：按动作切换点、视角切换点、对象切换点切分子镜头，为每个段落补全完整字段后调用 `save_storyboards` 保存。达到段数上限后停止，不要再追加批次

## 节奏分层时长

按段落功能确定时长，全部不超过 `video_generation.duration_max`：

| 段落类型 | 时长 | 说明 |
|---|---|---|
| 过渡段 | 靠近 duration_min | 赶路、空镜、环境建立、转场 |
| 叙事段 | 靠近 typical_shot | 常规剧情推进、对话 |
| 爆点段 | 不超过 duration_max | 特写、规则揭示、情感爆发、反转；子镜头节奏在上限内放慢 |

## 台词时长下限（硬规则）

**段落时长 ≥ 段内台词与旁白总字数（写在 description 中的部分）÷ `dialogue_chars_per_second` + `acting_padding_seconds`，且不得超过 `duration_max`**
...
10. **时长** `duration`：必须在 `duration_min–duration_max` 之间，且满足台词时长下限
