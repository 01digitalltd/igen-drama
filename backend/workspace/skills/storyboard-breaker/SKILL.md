...
**段落时长 ≥ 段内对白总字数 ÷ 4.5字/秒 + 2秒表演余量**

装不下的对白必须拆到下一个段落，不允许把演不完的对白塞进一个段落。
...
7. **动作** `action`：按 `【镜头1】…【镜头2】…` 逐子镜头组织，每个子镜头写清谁 + 具体怎么做 + 肢体细节 + 表情
8. **对话** `dialogue`：该段落的完整对话
9. **画面结果** `result`：段落结尾的即时后果 + 视觉细节
10. **氛围** `atmosphere`：光线 + 色调 + 声音 + 整体氛围
11. **时长** `duration`：段落总时长 8-15 秒，且满足对白时长下限
12. **画面描述** `description`：按 `【镜头N】` 逐子镜头描述观众实际看到的画面
13. **场景关联**：若能匹配已有场景，必须填写 `scene_id`
14. **角色关联**：填写 `character_ids`，绑定当前段落涉及的 0 到多个角色
15. **道具关联**：填写 `prop_ids`，绑定当前段落出现的关键道具（0 到多个）
...
- `description` 要适合人读，按子镜头详细描述观众实际看到的画面内容
