...
test('video prompt batch service runs per-shot async agent loop', () => {
  ...
  assert.match(svc, /voVoiceInstruction\(narratorVoice\)/)
  assert.match(svc, /visualStyleInstruction\(styleValue\)/)
  assert.match(svc, /getDramaDialogueLanguage\(dramaId\)/)
  assert.match(svc, /getDramaVoVoice\(dramaId\)/)
  assert.match(svc, /getDramaStyleValue\(dramaId\)/)
  assert.match(svc, /rewriteNarratorLabels\(drafted, narratorVoice\)/)
