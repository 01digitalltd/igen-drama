            ...
            <div class="prod-section-bar">
              ...
              <div class="ml-auto flex gap-1 items-center">
                <label class="dim" style="font-size:12px;display:flex;align-items:center;gap:6px">
                  对白语言
                  <select class="input" style="width:auto;padding:2px 8px;font-size:12px" :value="dramaDialogueLanguage" @change="setDialogueLanguage($event.target.value)">
                ...
                <button v-if="pendingVideoIds.length" class="btn btn-sm" @click="stopAllVideos">停止全部</button>
                <button v-else class="btn btn-sm" :disabled="!sbs.length" @click="batchVideos">
              ...
              <section class="video-task-list">
                ...
                >
                  ...
                  <button
                    ...
                    @click.stop="videoTaskState(task.storyboard) === 'pending' ? cancelVid(task.storyboard) : genVid(task.storyboard)"
                  >
                    <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><polygon points="23 7 16 12 23 17 23 7"/><rect x="1" y="5" width="15" height="14" rx="2" ry="2"/></svg>
              ...
              <aside class="video-task-inspector">
                <div class="video-inspector-body">
                  ...
                  <button
                    ...
                    @click="videoTaskState(selectedSb) === 'pending' ? cancelVid(selectedSb) : genVid(selectedSb)"
                  >
...
function videoTaskStatusLabel(sb) {
  ...
  if (state === 'pending') return '排队/生成中'
...
async function setDialogueLanguage(code) {
  ...
  if (next === dramaDialogueLanguage.value || !drama.value) return
  const prevLang = drama.value.dialogue_language || drama.value.dialogueLanguage
  drama.value = { ...drama.value, dialogue_language: next, dialogueLanguage: next }
  try {
    await dramaAPI.update(dramaId, { dialogue_language: next })
    if (sbs.value.some(sb => (sb.video_prompt || sb.videoPrompt || '').trim())) {
      toast.info('对白语言已更新。请重新生成视频提示词，否则旧台词语言不会变。')
    }
  } catch (e) {
...
function stopAllVideos() {
  const ids = [...new Set([
    ...Object.values(videoTaskIds.value),
    ...genTasks.value.filter(t => t.type === 'video' && ['queued', 'pending', 'processing'].includes(t.status)).map(t => t.id),
  ])].filter(Boolean)
  ids.forEach(id => {
    void taskAPI.cancel(id).catch(e => toast.error(e.message || '停止失败'))
  })
