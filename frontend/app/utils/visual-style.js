...
export function visualStyleInstruction(styleRaw) {
  ...
  if (key === '3d') {
    return `【视觉风格｜必须遵守】本项目是${label}。video_prompt 全程是 3D Q版／Chibi CG（头大身小、圆润可爱、游戏引擎渲染）。@场景/@角色/@道具 只提供外形、包装与 Logo，必须写成「转成 3D Chibi CG」，禁止按实拍照片还原办公室或桌面广告。禁止写成真人实拍或半写实成人比例。每一段画面都保持这个画风。`
  }
