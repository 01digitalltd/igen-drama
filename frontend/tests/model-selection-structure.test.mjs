...
test('workbench offers model selectors for rewrite, image and video generation', () => {
  ...
  assert.match(page, /v-model="rewriteModel"/)
  assert.match(page, /v-model="imageModel"/)
  assert.match(page, /v-model="videoModel"/)
  assert.doesNotMatch(page, /<select v-model="rewriteModel"/)
...
test('selected models are sent with rewrite, image and video generation requests', () => {
  ...
  assert.match(page, /runAgent\('script_rewriter'[\s\S]*?rewriteModel\.value \|\| undefined, ownerConfigId\(textModelOptions\.value, rewriteModel\.value\)\)/)
  // 图片生成透传模型与配置
  assert.match(useApi, /generateImage: \(id: number, episodeId: number, model\?: string, configId\?: number\)/)
  assert.match(useApi, /config_id: configId \|\| undefined/)
  assert.match(page, /characterAPI\.generateImage\(id, epId\.value, imageModel\.value \|\| undefined, ownerConfigId\(imageModelOptions\.value, imageModel\.value\)\)/)
  assert.match(page, /sceneAPI\.generateImage\(id, epId\.value, imageModel\.value \|\| undefined, ownerConfigId\(imageModelOptions\.value, imageModel\.value\)\)/)
  assert.match(page, /characterAPI\.batchImages\(ids, epId\.value, imageModel\.value \|\| undefined, ownerConfigId\(imageModelOptions\.value, imageModel\.value\)\)/)
  // 视频生成透传模型与配置
